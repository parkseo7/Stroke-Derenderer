"""Methods for training an encoder-decoder image captioning model. Modified
version to utilize DDP.
"""
...
def setup_directories(vargs, mkdir=True):
...
def prepare_datasets(vargs, dirs):
    ...
    train_dl = DataLoader(train_dataset,
                          ...
                          ) # Keep this False, strains the GPU
...
def prepare_models(vargs, device):
    ...
    # Display models stats before wrapping in DDP:
    if device == 0:
        p = encoder.trainable_parameters
        print(f"Number of trainable encoder parameters: {p}")
...
def main(rank, world_size, vargs):
    """Main script to set-up and train a model. Into mp.spawn, the arguments
    are (world_size, vargs).
    """
    ...
    dirs = setup_directories(vargs, mkdir=(device == 0))
    ...
    all_models = prepare_models(vargs, device)
    ...
    for epoch in range(1, epochs+1):
        ...
        display_log({
            "epoch": epoch,
            "device": device,
            "tloss": log_entry["tloss"],
            "vloss": log_entry["vloss"],
            "lr": log_entry["decoder_lr"]
        })
        ...
        # Save the models:
        if epoch % save_per_epoch == 0 and device == 0:
